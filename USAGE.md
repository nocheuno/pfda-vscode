# PFDA Explorer Extension Usage Guide

## File Explorer Functionality

### Drag and Drop File Uploads

You can easily upload files and folders to PFDA using drag-and-drop:

1. Simply drag files or folders from your local file system
2. Drop them onto the desired folder in the PFDA File Explorer
3. Confirm the upload when prompted
4. A progress indicator will show in the status bar and notification area

This works with:
- Single files
- Multiple files
- Folders (including nested folders and files)

The upload will use the pfda CLI tool with the following options:
```
upload-file <files...> [-space-id <ID>] [-folder-id <ID>] 
```

Where:
- `-space-id` is determined by the currently selected space
- `-folder-id` is determined by the folder where you dropped the files

### Context Menu Upload

You can also use the context menu to upload files:

1. Right-click on a folder in the PFDA File Explorer
2. Select "Upload Files to PFDA" from the menu
3. Select the files/folders to upload in the dialog
4. Files will be uploaded to the selected folder

## Other File Operations

- **Download Files**: Right-click on a file and select "Download"
- **Copy File ID**: Right-click on a file and select "Copy File DXID"
- **Create Folder**: Use the "New Folder" command
- **Delete Files/Folders**: Right-click and select "Delete"
